...
class WeatherGenerator {
    ...
    generateCloudiness = (precipitation: Precipitation): Cloudiness => {
        ...
        if (precipitation.mmperH > 0.5) return new Cloudiness(this.randomNormal(50, 25))
    ...
    GenerateRandomWeather = (): Weather => {
        ...
        let effects = []
        if (cloudiness.level == 2) effects.push(getEffect("heavy clouds")); else effects.push(getEffect("clear skies"))
        if (temperature.level == -2) effects.push(getEffect("freezing cold")); else if (temperature.level == 2) effects.push(getEffect("scorching heat"));
    ...
    GenerateRandomWeatherReport = (datetime: Date): WeatherReport => {
        const newWeatherReport: WeatherReport = { dateTime: datetime, weather: this.GenerateRandomWeather() }
